...
class CurrencyExcange:
    ...
    def get_price(val, keys):
        ...
        if base not in keys.keys():
            raise APIException(f"Некорректное или отсутствующее в базе название валюты '{base}'!")
        ...
        try:
            try:
                h = requests.get(f"https://openexchangerates.org/api/latest.json?"
                                 f"app_id={openexchanger_token}"
                                 f"base=USD&symbols={keys[quote]},{keys[base]}")
                ...
                result += f"По данным openexchangerates.org:\n" \
                          f"{amount} {keys[quote]} = {openexrates} {keys[base]}\n\n"
            except Exception:
        ...
        try:
            try:
                ...
                result += f"По данным exchangeratesapi.io:\n" \
                          f"{amount} {keys[quote]} = {exratesio} {keys[base]}\n\n"
            except Exception:
                raise HTMLException(f"Ошибка сервиса exchangeratesapi.io!\n\n")
        except HTMLException as e:
            result += f"{e}"
        ...
        try:
            try:
                ...
                result += f"По данным cryptocompare.com:\n" \
